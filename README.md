...
## עדכון קוד דרך הוויפי (OTA) - בלי USB

אחרי ההעלאה הראשונה דרך USB, אפשר לעדכן את הקוד דרך הרשת - **בתנאי
שהמחשב שממנו מעלים נמצא באותה רשת וויפי כמו הבקר** (זו לא העלאה מכל
מקום בעולם, רק כשאת בבית).

1. ודאי שיש `OTA_PASSWORD` מוגדר ב-`secrets.h` שלך
2. חברי את המחשב לאותה רשת וויפי כמו הבקר
3. ב-Arduino IDE: **Tools → Port** - אחרי כמה שניות אמור להופיע פורט
   רשת בשם `esp32_home at <IP>` (בנוסף לפורטי ה-USB) - בחרי אותו
4. לחצי **Upload** כרגיל - הפעם זה יעבור דרך הוויפי, בלי USB
...
3. בטאב **Rules** של ה-Realtime Database, הדביקי את התוכן של
   [`database.rules.json`](database.rules.json) (כולל נתיב `speedtests`) ופרסמי (Publish).
